...
def iter_files(root: str) -> Iterable[str]:
    for dirpath, _, filenames in os.walk(root):
        for fn in filenames:
            if fn.startswith("."):
                continue
            # 忽略遗留/预览/原始 dump 文件，避免重复入库或把大 JSON 当纯文本 ingest
            fn_l = fn.lower()
            dp_l = dirpath.replace("\\", "/").lower()
            if fn_l.startswith("twitter_") and (fn_l.endswith("_rapid.md") or fn_l.endswith("_rapid.json")):
                continue
            if "/data/raw/x/" in dp_l:
                if fn_l.startswith("tweets_") and fn_l.endswith(".md"):
                    continue
                if fn_l.startswith("raw_") and fn_l.endswith(".json"):
                    continue
...
def ingest(full: bool = False) -> Dict[str, Any]:
    # Card 6：确保 data/ 目录存在（尤其是 data/corpus.jsonl 的父目录）
    try:
